<?php
// wird ins login/InsTokenSendEmail.php integriert)
...
//$path=$_SERVER['SERVER_NAME']; // z.B.: projectboard.azurewebsites.net
			...
			$path = substr($actual_link, 0, -22); // /sgPHPmailer_check.php = 22
			$loginlogin = "/login.php";			
			$loginPage = $path.$loginlogin;
...
if(!$mail->Send()) {
  ...
  <br>
  Please check the credentials you entered for your email-provider in PDO/email-provider.php <br>
<br> 
   Error: " . $mail->ErrorInfo;
} else {
  echo 'Notification Email for checking email-functionality was sent successfully !<br>
Return to login-page: <a href='.$loginPage.'>LOGIN</a>  
  ';
